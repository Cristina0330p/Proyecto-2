...
class MyGraphCanvas(FigureCanvas):
    ...
    def graficar_gatos(self,datos):
        ...
        self.axes.set_xlabel("Frecuencia")
...
class InterfazGrafico(QMainWindow):
    ...
    def setup(self):
        ...
        self.nooverlap_2.setEnabled(False)
        self.scaling.setEnabled(False)
        
        self.nperseg.setEnabled(False)
        self.reload.setEnabled(False)
        self.reload_2.setEnabled(False)
        ...
        self.reload_2.clicked.connect(self.welchprom)
        self.multipaper.clicked.connect(self.calcular_multipaper)
        self.boton_aumentar_2.clicked.connect(self.aumentar_senal)
        self.boton_disminuir_2.clicked.connect(self.disminuir_senal)
        self.boton_adelante_2.clicked.connect(self.adelante_senal)
        self.boton_atras_2.clicked.connect(self.atrasar_senal)
    ...
    def cargar_senal(self):
        archivo_cargado, name= QFileDialog.getOpenFileName(self, "Abrir señal","","Todos los archivos (*);;Archivos mat (*.mat)*")
        if archivo_cargado != "":
    ...
    def load_senal(self):
        ...
        if(int(self.senalSeleccionada.text())<=len(self.senales.keys()) and int(self.senalSeleccionada.text())>0):
            ...
            self.nooverlap_2.setEnabled(True)
            self.scaling.setEnabled(True)
            self.boton_aumentar_2.setEnabled(True)
            self.boton_disminuir_2.setEnabled(True)
            self.boton_adelante_2.setEnabled(True)
            self.boton_atras_2.setEnabled(True)
    ...
    def welch(self):
        ...
        if(int(self.ventana.text())>0 and int(self.ventana.text())<6  and int(self.scaling.text())>0 and int(self.scaling.text())<3 and float(self.fs.text())>0 and float(self.fs.text())<500 ):
            print(self.fs.text())
           ...
           #graficar utilizando el controlador
            ...
            p ,  pxx  =  signal.welch(self.senalSelected , float(self.fs.text()) ,diccionario1.get(self.ventana.text()), nfft = nperseg, noverlap_2 = float(self.nooverlap_2.text()), scaling=diccionario2.get(self.scaling.text()))
            print(pxx)
        ...
        if float(self.nooverlap.text())<=((float(self.nperseg.text()))/2):
            print('Este valor de solapmiento que es definido ,es propio para no sobreponer las señales del analisis welch.')
            print('En el caso de querer un solapamiento al 50% nprseg*0.5 y el no overlap segira sieno la mitad pero se educe en 0.5')
            print('Para sobrepone el doble se recomienda un nperseg*2 y nooverlap*2')
        else:
            print("Fueraa de rango\nRazones:")
            
            if float(self.fs.text())>500:
                print('La frecuencia a usar debe estar en el rango conocido para las senales biologicas')
            if (int(self.ventana.text()))>6:
                print('Debe seleccionar una ventana que este en la lista') 
            if (int(self.scaling.text()))>=3:
                print('Opcion del welch fuera de rango')
           
            self.ventana.clear()
            self.fs.clear()
            self.nperseg.clear()
            self.nooverlap.clear()
            self.scaling.clear()
    def welchprom(self):
        diccionario1_1 = dict()
        diccionario1_1 = {"1":"hamming","2":"hanning","3":"blackman","4":"flattop","5":"boxcar"}
        diccionario2_2 = dict()
        diccionario2_2 = {"1":"density","2":"spectrum"}
        
        if( float(self.fs.text())>0 and float(self.fs.text())<500 and int(self.ventana.text())>0 and int(self.ventana.text())<6 and int(self.scaling.text())>0 and int(self.scaling.text())<3) :
           
            print(self.fs.text())
            print(diccionario1_1.get(self.ventana.text()))
            print(diccionario2_2.get(self.scaling.text()))
            x=np.mean(self.senalSelected)
            
            
            p=(self.senalSelected)-x
            c ,  Cxx_den  =  signal.welch(p , 
                                          float(self.fs.text())
                                          ,diccionario1_1.get(self.ventana.text()),
                                          nperseg= (float(self.fs.text()))*float(5),
                                          noverlap=0, 
                                          scaling=diccionario2_2.get(self.scaling.text()))
            print(Cxx_den)
            self.__coordinador.recibirDatosSenal(Cxx_den)
            self.__x_min=0
            self.__x_max=2000            
           #graficar utilizando el controlador
            self.__sc2.graficar_gatos(self.__coordinador.devolverDatosSenal(self.__x_min,self.__x_max))
