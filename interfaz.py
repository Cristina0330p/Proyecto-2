...
class MyGraphCanvas(FigureCanvas):
    ...
    def graficar_gatos(self,datos):
        ...
        #voy a graficar en un mismo plano varias senales que no queden superpuestas cuando uso plot me pone las graficas en un mismo grafico
...
class InterfazGrafico(QMainWindow):
    ...
    def cargar_senal(self):
        archivo_cargado, name= QFileDialog.getOpenFileName(self, "Abrir senal","","Todos los archivos (*);;Archivos mat (*.mat)*")
        if archivo_cargado != "":
    ...
    def welch(self):
        ...
        if(int(self.ventana.text())>0 and int(self.ventana.text())<6  and int(self.scaling.text())>0 and int(self.scaling.text())<3 and float(self.fs.text())>0 and float(self.fs.text())<500 and float(self.nooverlap.text())<(float(self.nperseg.text()))/2):
            print(self.fs.text())
           ...
           #graficar utilizando el controlador
            ...
            if float(self.nooverlap.text())>=((float(self.nperseg.text()))/2):
                print('No se sugiere este valor de solapamieno')
